...
func CreateObject(w donburi.World, position math.Vec2, size math.Vec2, name string) *donburi.Entry {
	e := archetypes.Interaction.Create(w)

	transform := components.Transform.Get(e)
	transform.LocalPosition = position

	object := components.NewObject(e, int(size.X), int(size.Y), tags.ResolvTagInteractive)
